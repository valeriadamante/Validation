...
class histomaker:
    ...
    subdirectories_names= ["TrackingRecHitV/"]
    #subdirectories_names= ["RecHit/", "RecHitV/", "Digi/", "Cluster/", "TrackingRecHitV/"]
    #subdirectories_names= ["Digi/", "Cluster/","TrackingRecHitV/"]
    #subdirectories = {"OT": subdirectories_names}
    ...
    def search_paths(self):
        ...
        with open(self.paths_file_name, 'r') as paths_file:
            #print(self.paths_file_name)
            for line in paths_file.read().splitlines():
                k = 0
                for i in full_paths:
                    if(len(self.variables)==0):
                        if i in line: # and "Barrel/Layer1" in line :
    ...
    def draw_histograms(self):
        ...
        for path in self.paths:
            position = [0.72, 0.94, 0.8, 0.9]
            #position = [0.72, 0.94, 0.715, 0.935]
            ...
            for file in self.files_open:
                ...
                htemp = file.Get(path)
                #print(path)
                htemp_prev = file.Get(previous_path)
                htemp.SetName(self.get_hist_title(self.input_files[i]))
                print (self.get_hist_title(self.input_files[i]))
                options = ""
                if(self.norm and htemp.GetEntries()>0):
                    htemp.Scale(1000/(htemp.GetEntries()))
                if "TH2" in htemp.ClassName():
                    continue
                if "TH1" in htemp.ClassName():
                    print "i = ", i
                    alpha = float(float(i+len(self.color_list))/float(2*len(self.color_list)))

                    if(i==0):
                        alpha = 1
                    print "alpha = ", alpha
                    htemp.SetLineColorAlpha(self.color_list[i], alpha)
                    htemp.SetLineWidth(2)
                    htemp.SetMaximum(htemp.GetMaximum()*1.25)
                    options = options+ "hist"
                    #if i > 0:
                    #    rp = TRatioPlot(htemp, htemp_prev);
                    #    rp.Draw()
...
dir_dict["lxplus"]={}
dir_dict["lxplus"]["input"]="/afs/cern.ch/user/g/garamire/work/public/DQMTracker/forValeria/"
...
#dir_dict["lxplus"]["output"]="/eos/home-v/vdamante/www/phase2validation/"
dir_dict["lxplus"]["output"]="/afs/cern.ch/work/v/vdamante/public/"

input_dir = dir_dict[args.machine]["input"]
out_dir =  dir_dict[args.machine]['output']

files_to_read = []
files_to_read.append(input_dir+args.reference)
for i in args.targets:
    files_to_read.append(input_dir+i)
# **** create txt file with all paths  ****
input_files_str = ",".join(files_to_read)
...
make_histogram.paths_file_name=args.txtname+".txt"                            # file name with paths
